...
int main() 
{
	...
	// Set i to equal 1 so that I can perform the % on i to check for positive or negative
	for (int i = 1; i < 5000000000; i++)
	{
		currentCalculation = (Numerator / Denominator);
		Denominator += 2;
		
		// If the index number is 1 e.g. starting number then add current calculation and skip straight to the next iteration
		if (i == 1) {
			piSum = currentCalculation;
			continue; 
		}
			
		// If the index number is positive then subtract the previous calculation.
		else if (i % 2 == 0) {
			piSum -= currentCalculation;
		}
			
		// If the index number is negative then add the previous calculation.
		else if (i % 2 != 0) { 
			piSum += currentCalculation;
		}
	}
